...
from noc.models.clip.clip import tokenize
...
class CLIPCapDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        c_data = tokenize(self.prefix + c_data, truncate=True).squeeze()
...
class CLIPImageDataset(torch.utils.data.Dataset):
    def __init__(self, data):
        self._REGIONAL_DS = ["vg_attr", "vg_rel", "vg_region"]
        self.data = data # list of annotation dictionary
    ...
    def __getitem__(self, idx):
        c_data = self.data[idx]
        image = Image.open(c_data['imgpath'])

        if c_data['ds_name'] in self._REGIONAL_DS:
            image = image.crop(c_data['bbox'])
...
def extract_all_captions(captions, model, device, batch_size=256, num_workers=8):
    ...
    with torch.no_grad():
        for b in tqdm.tqdm(data):
            ...
            all_text_features.append(model.encode_text(b, use_fp16=True).cpu().numpy())
